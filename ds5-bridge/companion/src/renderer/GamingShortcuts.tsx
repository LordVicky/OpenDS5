...
function ControllerPreview({ settings, status, selectedGameName }: { settings: GamingShortcutsSettings; status: BridgeSnapshot | null; selectedGameName?: string }) {
  ...
  return <section className="gaming-shortcuts-preview system-card" aria-label="Controller preview">
    <div className="gaming-preview-heading"><div><h3>{selectedGameName ? `${selectedGameName} shortcuts` : 'Your PS button layout'}</h3></div><span className={`gaming-controller-state ${status?.status?.controllerConnected ? 'connected' : ''}`}><span aria-hidden="true" />{connection}</span></div>
...
export function GamingShortcuts({ active, profiles, activeProfileId, snapshot = null }: { active: boolean; profiles: TriggerProfile[]; activeProfileId: string | null; snapshot?: BridgeSnapshot | null }) {
  ...
  return <div className={`control-page gaming-shortcuts-page ${active ? 'active' : ''}`} role="tabpanel" id="control-panel-gaming-shortcuts" aria-labelledby="control-tab-gaming-shortcuts" aria-hidden={!active}>
    ...
    </div><div className="gaming-shortcuts-rail"><ControllerPreview settings={settings} status={snapshot} selectedGameName={selectedGameName} /><section className="system-card gaming-shortcuts-card gaming-provider-status" aria-label="Provider status"><span>Screenshot: {capabilities?.screenshot.join(', ') || 'Unavailable'}</span><span>Recording: {capabilities?.recording.join(', ') || 'Unavailable'}</span><span>HUD: {capabilities?.hud.join(', ') || 'Unavailable'}</span></section></div></div>
