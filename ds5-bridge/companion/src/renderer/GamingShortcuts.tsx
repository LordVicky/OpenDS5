...
function ActionEditor({ action, onChange, capabilities }: {
  ...
  const providerOptions = action.type === 'screenshot' ? capabilities?.screenshot ?? []
    : action.type === 'recording-toggle' ? capabilities?.recording ?? []
      : action.type === 'performance-hud-toggle' ? capabilities?.hud ?? [] : capabilities?.keyboard ?? [];
...
export function GamingShortcuts({ active, profiles, activeProfileId }: { active: boolean; profiles: TriggerProfile[]; activeProfileId: string | null }) {
  ...
  const addChord = (existing: GamingShortcutsSettings['chords']) => {
    const button = BUTTONS.find((candidate) => !existing.some((chord) => chord.button === candidate.value))?.value;
    return button ? [...existing, { button, action: { type: 'none' } as GamingShortcutAction }] : existing;
  };
  ...
  const defaultPreset = useMemo(() => ({ ...DEFAULT_GAMING_SHORTCUTS_SETTINGS, enabled: true, psPressAction: 'enter-shortcut-mode' as const,
    directChordsEnabled: true, shortcutModeEnabled: true,
    doublePress: { type: 'switch-application', direction: 'previous' } as GamingShortcutAction,
    chords: [
      { button: 'create' as const, action: { type: 'screenshot', provider: 'auto' } as GamingShortcutAction },
      { button: 'triangle' as const, action: { type: 'performance-hud-toggle', provider: 'auto' } as GamingShortcutAction },
      { button: 'mute' as const, action: { type: 'microphone-mute-toggle' } as GamingShortcutAction },
      { button: 'dpad-up' as const, action: { type: 'volume', direction: 'up' } as GamingShortcutAction },
      { button: 'dpad-down' as const, action: { type: 'volume', direction: 'down' } as GamingShortcutAction },
      { button: 'dpad-left' as const, action: { type: 'switch-application', direction: 'previous' } as GamingShortcutAction },
      { button: 'dpad-right' as const, action: { type: 'switch-application', direction: 'next' } as GamingShortcutAction }
      ,{ button: 'touchpad' as const, action: { type: 'on-screen-keyboard', provider: 'auto' } as GamingShortcutAction }
    ]
  }), []);
  return <div className={`control-page gaming-shortcuts-page ${active ? 'active' : ''}`} role="tabpanel" id="control-panel-gaming-shortcuts" aria-labelledby="control-tab-gaming-shortcuts" aria-hidden={!active}>
    ...
    <section className="system-card gaming-shortcuts-card">
      <div className="gaming-shortcuts-toggle"><div><strong>Enable Gaming Shortcuts</strong><span>Steam or a game may also receive the PS button.</span></div><button type="button" role="switch" aria-checked={settings.enabled} className={`switch ${settings.enabled ? 'on' : ''}`} onClick={() => update({ ...settings, enabled: !settings.enabled })}><span /></button></div>
      <div className="gaming-shortcut-grid">
        <label className="gaming-shortcut-row"><span>PS press behavior</span><select aria-label="PS press behavior" value={settings.psPressAction} onChange={(event) => update({ ...settings, psPressAction: event.target.value as GamingShortcutsSettings['psPressAction'] })}><option value="enter-shortcut-mode">Enter shortcut mode</option><option value="show-shortcut-reference">Show shortcut reference</option><option value="open-opends5">Open or focus OpenDS5</option><option value="none">No action</option></select></label>
        <label className="gaming-shortcut-row"><span>Direct PS chords</span><button type="button" role="switch" aria-checked={settings.directChordsEnabled} className={`switch ${settings.directChordsEnabled ? 'on' : ''}`} onClick={() => update({ ...settings, directChordsEnabled: !settings.directChordsEnabled })}><span /></button></label>
        <label className="gaming-shortcut-row"><span>Temporary shortcut mode</span><button type="button" role="switch" aria-checked={settings.shortcutModeEnabled} className={`switch ${settings.shortcutModeEnabled ? 'on' : ''}`} onClick={() => update({ ...settings, shortcutModeEnabled: !settings.shortcutModeEnabled })}><span /></button></label>
        <label className="gaming-shortcut-row"><span>Success notifications</span><button type="button" role="switch" aria-checked={settings.showSuccessNotifications} className={`switch ${settings.showSuccessNotifications ? 'on' : ''}`} onClick={() => update({ ...settings, showSuccessNotifications: !settings.showSuccessNotifications })}><span /></button></label>
        <label className="gaming-shortcut-row"><span>Error notifications</span><button type="button" role="switch" aria-checked={settings.showErrorNotifications} className={`switch ${settings.showErrorNotifications ? 'on' : ''}`} onClick={() => update({ ...settings, showErrorNotifications: !settings.showErrorNotifications })}><span /></button></label>
        <button type="button" className="secondary-action" onClick={() => void window.bridge.previewGamingShortcutNotification()}>Preview shortcut notification</button>
        <button type="button" className="secondary-action" onClick={() => update(DEFAULT_GAMING_SHORTCUTS_SETTINGS)}>Reset Gaming Shortcuts</button>
      </div>
      <div className="gaming-shortcut-grid">
    ...
    <section className="system-card gaming-shortcuts-card"><div className="feature-card-title system-card-heading"><strong>PS chords</strong><button type="button" className="secondary-action" onClick={() => update({ ...settings, chords: addChord(settings.chords) })}>Add chord</button></div><p className="gaming-shortcuts-help">Hold PS and press another controller button. Direct chords take precedence over PS press gestures.</p>
    ...
    <section className="system-card gaming-shortcuts-card"><div className="feature-card-title system-card-heading"><strong>Timing</strong><button type="button" className="secondary-action" onClick={() => update(defaultPreset)}>Apply Linux Gaming preset</button></div><div className="gaming-timing-grid"><label>Double press window <input type="number" min="100" max="1000" step="10" value={settings.doublePressWindowMs} onChange={(event) => update({ ...settings, doublePressWindowMs: Number(event.target.value) })} /> ms</label><label>Hold threshold <input type="number" min="300" max="2000" step="10" value={settings.longPressThresholdMs} onChange={(event) => update({ ...settings, longPressThresholdMs: Number(event.target.value) })} /> ms</label><label>Chord window <input type="number" min="50" max="500" step="10" value={settings.chordWindowMs} onChange={(event) => update({ ...settings, chordWindowMs: Number(event.target.value) })} /> ms</label><label>Shortcut mode timeout <input type="number" min="1000" max="10000" step="100" value={settings.shortcutModeTimeoutMs} onChange={(event) => update({ ...settings, shortcutModeTimeoutMs: Number(event.target.value) })} /> ms</label></div></section>
    <section className="system-card gaming-shortcuts-card"><div className="feature-card-title system-card-heading"><strong>Per-game overrides</strong>{selectedOverride && <button type="button" className="secondary-action" onClick={clearGameOverride}>Use global settings</button>}</div>
      {gameProfiles.length === 0 ? <p className="muted-copy">Create a Game Profile to customize shortcuts for a specific game.</p> : <>
        ...
        {selectedGameId && <>
          ...
          <div className="feature-card-title system-card-heading"><strong>PS chords</strong><button type="button" className="secondary-action" onClick={() => setGameOverride({ ...selectedOverride, chords: addChord(selectedOverride?.chords ?? []) })}>Add chord</button></div>
